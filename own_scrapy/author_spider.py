...
class AuthorsSpider(scrapy.Spider):
    ...
    author_urls = []
    custom_settings = {'FEED_FORMAT': 'json', 'FEED_URI': os.path.join('data', 'authors.json'), 'LOG_LEVEL': 'ERROR'}
    ...
    def parse(self, response):
        for author in response.xpath("/html//div[@class='quote']/span"):
            ...
            if url and url not in self.author_urls:
                ...
                yield scrapy.Request(url=self.start_urls[0] + url, callback=self.get_author)
    ...
    def get_author(self, response):
        ...
        return {
            'fullname': author.xpath("h3/text()").get().replace('\n', '').replace('-', ' ').strip(),
